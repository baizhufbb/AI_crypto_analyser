...
5. **自动报告**：返回结构清晰的分析结果，**必须严格遵循 `docs/ai_autonomous_analysis.md` 中定义的两阶段分析框架**，特别注意：
   - **阶段0：必须完成所有数据获取和扫描清单**
   - **阶段1：必须分析所有定义的数据维度（多周期K线、技术指标、市场微观结构等）**
   - **阶段2：必须给出完整的执行方案（入场/止损/止盈/仓位/胜率/风险点）**
...
### 2. 执行前的默认约束与偏好读取
 ...
 当用户要求你"通过本项目帮忙操作"（例如：看大盘、筛选合约、分析一组合约等）时，**默认行为是：严格遵循 `docs/ai_autonomous_analysis.md` 中定义的两阶段分析框架**：
- **阶段0**：强制数据扫描清单（确保所有数据已获取）
- **阶段1**：深度市场分析（必须分析所有定义的数据维度）
- **阶段2**：综合决策与执行方案（基于阶段1给出具体方案）
...
- **信息增强**：
 ...
 - **数据获取**：
  ...
  - 按 `docs/ai_autonomous_analysis.md` 中定义的阶段0流程，从 snapshot 的全量 `tickers` 中构建候选池，并依据阶段1的数据维度要求，调用 `fetch_klines.py` 对入选标的批量获取多周期详细数据，完成阶段1的深度分析后，再输出阶段2的最终可执行方案。
